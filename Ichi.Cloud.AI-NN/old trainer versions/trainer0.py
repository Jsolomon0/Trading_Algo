...
# ============================================
# Feature Engineering Functions (same as live)
# ============================================
...
def train_agent(features_df, num_episodes=100, max_steps_per_episode=1000):
    ...
    Args:
        features_df: DataFrame with features and close prices
    ...
    # Prepare feature matrix (excluding 'close' column for features)
    ...
    for episode in range(num_episodes):
        ...
        # Randomly start somewhere in the data (but leave room for sequence + episode)
        ...
        for step in range(max_steps_per_episode):
            ...
            # Train agent
            if len(agent.replay_buffer) >= 32 and step % 10 == 0:
...
def backtest_agent(agent, features_df):
    """
    ...
    for i in range(SEQUENCE_LENGTH, len(feature_matrix)):
        ...
        action, action_name, action_prob = agent.select_action(state_sequence, deterministic=True)
    ...
    print(f"  Total Trades: {num_trades} (Buy: {num_buys}, Sell: {num_sells})")
...
def plot_results(training_rewards, training_portfolios, backtest_results):
    ...
    axes[1, 1].plot(backtest_results['timestamps'], cumulative_rewards)
    axes[1, 1].set_title('Cumulative Rewards (Backtest)')
...
def main():
    ...
    # Step 4: Backtest
    backtest_results = backtest_agent(agent, features_df)
    
    # Step 5: Visualize
    plot_results(episode_rewards, episode_portfolios, backtest_results)
